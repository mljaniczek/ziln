...
infer_Z_mj <- function(X, seq_depth = "TS") {
  ...
  # seems to breakdown when a variable doesn't have ANY zeros so i'm adding a condition
  for(i in 1:d){
    if(sum(nz[,i]) == d){
      Z[,i] = Z[,i] # is it true we just want the original value for non-zeros??
    }
    else if(sum(nz[,i]) > 1){ # only will do this for variables which have at least more than one nonzero val
      mu = mean(log_ratios[is.finite(log_ratios)[,i],i]) # mean of non-zero vals
     # general purpose optimization algorithm
       o = optim(c(0, 1), log_likelihood, log_x = X_clr[,i], a  =  a[i]) # why using X_clr in this?
      par = o$par
